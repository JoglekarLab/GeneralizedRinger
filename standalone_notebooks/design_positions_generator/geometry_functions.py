...
def get_all_pairs (pdb_path, omit_chain_pairs = []):
    ...
    for i in range(len(pairs)):
        ...
        # print (pairs[i][0], pairs[i][1], 'lengths:', len(CA_coords_by_chain[pairs[i][0]]), len(CA_coords_by_chain[pairs[i][1]])) # For debugging purposes
    return pairs

def general_get_contacts(pairs, min_distance=5, max_distance=10):
    '''
